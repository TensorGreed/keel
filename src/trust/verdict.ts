...
export function evaluatePolicy(facts: VerdictFacts, policy: Policy): { verdict: VerdictLevel; reasons: VerdictReason[] } {
  ...
  } else if (sim.status === "runner-unsupported" || sim.status === "runner-unavailable" || sim.status === "environment-error") {
    // We selected the tests but couldn't execute them here: no runner for the language, the runner
    // isn't installed (e.g. pytest missing), or its environment couldn't be prepared (e.g. a Go
    // toolchain download failed). Don't pretend it passed; don't hard-block either — warn that this
    // change is unverified by the sim.
